...
public class GuiAlloyingMachine extends GuiContainer {
	...
	protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY) {
		...
		if ()
		drawFluids(((NonNullList<FluidTank>) this.tileentity
				.getCapability(CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY, null)), true);
...
//		this.drawTexturedModalRect(this.guiLeft + 59, this.guiTop + 32, 176, 14, l - 1, 22);
	...
	private void drawFluids(NonNullList<FluidTank> fluidTanks, boolean bindTexture) {
		drawFluid(fluidTanks.get(0), 10, 19, 12, 50, bindTexture);
		drawFluid(fluidTanks.get(1), 126, 19, 12, 50, bindTexture);
		drawFluid(fluidTanks.get(2), 68, 5, 12, 50, bindTexture);
	}
	...
	private void drawFluid(FluidTank fluidTank, int left, int top, int width, int height, boolean bindTexture) {
		if (fluidTank == null)
			...
			throw new NullPointerException("Do not try to draw null Fluid.");
		else if (fluidTank.getFluid().getFluid().getStill() == null)
			throw new NullPointerException("The texture of Fluid is null.");
		this.mc.getTextureManager().bindTexture(fluidTank.getFluid().getFluid().getStill());

		int tankScaled = this.getFluidTankScaled(height, fluidTank);
		this.drawTexturedModalRect(left + height - tankScaled, top, 0, 0, width, tankScaled);
		if (bindTexture)
			this.mc.getTextureManager().bindTexture(TEXTURES);
	}
